> Codex Safari note: this directory is a vendored, adapted SafariMCP runtime used by the `safari` Codex plugin. For Codex installation, setup, screenshots, and release notes, start with [`../../README.md`](../../README.md). The original SafariMCP README is preserved below for upstream context and attribution.

<div align="center">

<img src="social-preview.png" alt="Safari MCP Server — 81 native browser automation tools for AI agents on macOS" width="100%">
...
**81 tools** · **No Chrome/Puppeteer/Playwright needed** · **~5ms per command** · **60% less CPU than Chrome**

[Quick Start](#quick-start) · [All 81 Tools](#tools-81) · [Examples](examples/) · [Why Safari MCP?](#safari-mcp-vs-alternatives) · [Architecture](#architecture) · [Changelog](CHANGELOG.md)
...
Native WebKit. ~60% less CPU. Background operation. 81 tools. One `npx` command. macOS only.
...
- **81 tools** — navigation, clicks, forms, screenshots, network, storage, accessibility, and more
...
The postinstall script re-signs the helper with a stable identifier (`com.codex.local.safari-helper`) so this permission survives future upgrades — without that step, every `npm update` would silently revoke approval because the binary's adhoc-signed identifier changes per build.
