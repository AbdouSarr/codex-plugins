...
// Extension bridge is handled by index.js.

const execFileAsync = promisify(execFile);
const __dirname = dirname(fileURLToPath(import.meta.url));
const CODEX_SAFARI_HOME = process.env.CODEX_SAFARI_HOME || join(homedir(), ".codex-safari");
const HELPER_CODESIGN_IDENTIFIER = process.env.CODEX_SAFARI_HELPER_IDENTIFIER || "com.codex.local.safari-helper";
...
function ensureHelperCodesign(helperPath) {
  if (process.platform !== "darwin") return;
  try {
    const current = execFileSync("/bin/sh", ["-c", `codesign -dv --verbose=4 ${JSON.stringify(helperPath)} 2>&1`], { encoding: "utf8" });
    if (current.includes(`Identifier=${HELPER_CODESIGN_IDENTIFIER}`)) return;
  } catch {
    // Re-sign below when inspection fails; the helper may be ad-hoc signed.
  }
  try {
    const entitlements = join(__dirname, "safari-helper.entitlements");
    execFileSync("codesign", ["-s", "-", "-f", "--identifier", HELPER_CODESIGN_IDENTIFIER, "--entitlements", entitlements, helperPath], { stdio: "ignore" });
  } catch (err) {
    console.error(`[Safari MCP] unable to re-sign safari-helper with ${HELPER_CODESIGN_IDENTIFIER}: ${err.message}`);
  }
}
...
function startHelper() {
  ...
  try {
    ensureHelperCodesign(helperPath);
...
const _LOG_FILE = join(CODEX_SAFARI_HOME, "profile.log");
function _logProfile(msg) {
  const ts = new Date().toISOString().replace('T', ' ').substring(0, 19);
  const line = `[${ts}] ${msg}\n`;
  console.error(`[Safari MCP] ${msg}`);
  mkdir(CODEX_SAFARI_HOME, { recursive: true })
    .then(() => appendFile(_LOG_FILE, line))
    .catch(() => {});
}
...
async function _getSafariWindowGeometry() {
  ...
  // (title bar + URL bar + tab strip + optional bookmarks bar). A fixed 74 was wrong
...
const CODEX_SENSITIVE_RUN_SCRIPT_ACTIONS = new Set([
  "getLocalStorage",
  "setLocalStorage",
  "deleteLocalStorage",
  "getSessionStorage",
...
export async function runScript({ steps }) {
  ...
  for (const step of steps) {
    ...
    try {
      if (CODEX_SENSITIVE_RUN_SCRIPT_ACTIONS.has(action) && process.env.CODEX_SAFARI_ENABLE_STORAGE_TOOLS !== "1") {
        results.push({ action, error: "Storage, cookie, and IndexedDB actions are disabled unless CODEX_SAFARI_ENABLE_STORAGE_TOOLS=1." });
        continue;
      }
