...
const CODEX_SAFARI_HOME = process.env.CODEX_SAFARI_HOME || join(homedir(), ".codex-safari");
const OWNERSHIP_DIR = CODEX_SAFARI_HOME;
...
// Hydrated from ~/.codex-safari/owned-tabs.json so ownership survives MCP restarts.
...
function _trackTab(tabIndex, url, origin = "created") {
  _openedTabs.set(tabIndex, { url: url || "", openedAt: Date.now(), origin });
  _addOwnedURL(url);
}

async function _trackOpenedTabFromCurrentState(requestedUrl, result, origin = "created") {
  if (result?.tabIndex) {
    safari.setActiveTabIndex(result.tabIndex);
    _trackTab(result.tabIndex, requestedUrl || result.url || "");
    return result.tabIndex;
  }

  const candidateUrl = requestedUrl || result?.url || "";
  if (!candidateUrl) return null;

  try {
    const rawTabs = await safari.listTabs();
    const tabs = _parseTabs(rawTabs);
    const match = [...tabs].reverse().find(tab => {
      if (!tab.url) return false;
      if (tab.url === candidateUrl) return true;
      try {
        const tabUrl = new URL(tab.url);
        const wantedUrl = new URL(candidateUrl);
        return tabUrl.origin === wantedUrl.origin && tabUrl.pathname === wantedUrl.pathname;
      } catch {
        return false;
      }
    });
    if (match?.index != null) {
      ...
      _trackTab(match.index, match.url || candidateUrl, origin);
      return match.index;
    }
  } catch {}
...
async function _cleanupTabs() {
  ...
  const tabsToClose = [..._openedTabs.values()].filter(v => v.origin !== "claimed");
  if (tabsToClose.length === 0) {
    _openedTabs.clear();
    return;
  }
  console.error(`[Safari MCP] Cleanup: closing ${tabsToClose.length} MCP-opened tabs`);
  // Close by URL (not index) — indices shift as tabs are closed
  const urlsToClose = tabsToClose.map(v => v.url).filter(Boolean);
...
async function _closeOldestMCPTab() {
  ...
  for (const [idx, info] of _openedTabs) {
    if (info.origin === "claimed") continue;
...
const _preferAppleScript = !!process.env.SAFARI_PROFILE || process.env.CODEX_SAFARI_PREFER_APPLESCRIPT === "1";

// ========== EXTENSION BRIDGE (WebSocket + HTTP polling) ==========
const WS_PORT = parseInt(process.env.CODEX_SAFARI_WS_PORT || "9323", 10);
const HTTP_PORT = parseInt(process.env.CODEX_SAFARI_HTTP_PORT || "9324", 10);
...
// Read version from package.json to avoid manifest/package mismatch
const _pkgVersion = JSON.parse(readFileSync(join(dirname(fileURLToPath(import.meta.url)), 'package.json'), 'utf8')).version;
const server = new McpServer({
  name: "codex-safari",
  version: _pkgVersion,
  description: "Codex Safari automation - local WebKit control that keeps real Safari sessions",
});

const _sensitiveToolNames = new Set([
  "safari_get_cookies",
  "safari_set_cookie",
...
const _storageToolsEnabled = process.env.CODEX_SAFARI_ENABLE_STORAGE_TOOLS === "1";
const _registerTool = server.tool.bind(server);
server.tool = (name, ...args) => {
  if (_sensitiveToolNames.has(name) && !_storageToolsEnabled) {
    return undefined;
  }
  return _registerTool(name, ...args);
};
...
function _tabMatchesKeep(tab, keepEntries) {
  return keepEntries.some((entry) => {
    if (!entry || typeof entry !== "object") return false;
    if (entry.index != null && Number(entry.index) === Number(tab.index)) return true;
    if (entry.url && tab.url === entry.url) return true;
    if (entry.urlContains && tab.url && tab.url.includes(String(entry.urlContains))) return true;
    if (entry.titleContains && tab.title && tab.title.includes(String(entry.titleContains))) return true;
    return false;
  });
}

async function _runSetupStatus() {
  const status = {
    platform: platform(),
    stateDirectory: CODEX_SAFARI_HOME,
    ports: { websocket: WS_PORT, http: HTTP_PORT },
    storageToolsEnabled: _storageToolsEnabled,
    safariRunning: false,
    javascriptFromAppleEvents: false,
    helper: { path: join(dirname(fileURLToPath(import.meta.url)), "safari-helper"), signed: false, identifier: null },
    extension: { connected: _extensionConnected, host: _isExtensionHost, proxyMode: _primaryHasExtension },
    permissions: {
      automation: "unknown",
  ...
  try {
    ...
    status.safariRunning = true;
  } catch {
    status.notes.push("Safari is not running. Open Safari before using browser tools.");
  }
  try {
    const out = execFileSync("osascript", ["-e", 'tell application "Safari" to do JavaScript "1+1" in current tab of front window'], { encoding: "utf8", timeout: 5000 }).trim();
    status.javascriptFromAppleEvents = out === "2";
  } catch (err) {
    status.notes.push("Enable Safari > Develop > Allow JavaScript from Apple Events.");
    if (String(err?.message || err).includes("not authorized")) {
      status.permissions.automation = "needs approval";
    }
  }
  try {
    const out = execFileSync("/bin/sh", ["-c", `codesign -dv --verbose=4 ${JSON.stringify(status.helper.path)} 2>&1`], { encoding: "utf8" });
    const identifier = out.match(/Identifier=([^\n]+)/)?.[1] || null;
    status.helper.identifier = identifier;
    status.helper.signed = identifier === "com.codex.local.safari-helper";
    if (!status.helper.signed) status.notes.push("The Safari helper will be re-signed with com.codex.local.safari-helper on startup.");
  } catch {
    status.notes.push("codesign could not inspect the Safari helper.");
  }
  if (!_extensionConnected) {
    status.notes.push("The Codex Safari Bridge extension is not connected; AppleScript fallback remains available for supported operations.");
  }
  return status;
}

// ========== CODEX SETUP AND TAB HANDOFF ==========

server.tool(
  "safari_setup_status",
  "Check Safari automation prerequisites: Safari process, JavaScript from Apple Events, helper signing, extension connection, and plugin state paths.",
  {},
  async () => {
    const status = await _runSetupStatus();
...
server.tool(
  "safari_claim_tab",
  "Explicitly claim an existing Safari tab for Codex control by index or by title/URL substring. Claimed tabs are released, not closed, by safari_finalize_tabs.",
  {
    index: z.coerce.number().optional().describe("Tab index from safari_list_tabs"),
    urlContains: z.string().optional().describe("Claim the first tab whose URL contains this text"),
    titleContains: z.string().optional().describe("Claim the first tab whose title contains this text"),
  },
  async ({ index, urlContains, titleContains }) => {
    const rawTabs = await safari.listTabs();
    const tabs = _parseTabs(rawTabs);
    const tab = index != null
      ? tabs.find(t => Number(t.index) === Number(index))
      : tabs.find(t =>
          (!urlContains || (t.url && t.url.includes(urlContains))) &&
          (!titleContains || (t.title && t.title.includes(titleContains)))
        );
    if (!tab) {
      return { content: [{ type: "text", text: "No matching Safari tab found." }], isError: true };
    }
    const result = await safari.switchTab(tab.index);
    safari.setActiveTabIndex(tab.index);
    safari.setActiveTabURL(tab.url || "");
    _trackTab(tab.index, tab.url || "", "claimed");
    return { content: [{ type: "text", text: JSON.stringify({ claimed: tab, result }, null, 2) }] };
  }
);

server.tool(
  "safari_finalize_tabs",
  "Close Codex-created Safari tabs that are no longer needed and release claimed user tabs. Pass keep entries for tabs that should remain open.",
  {
    keep: z.array(z.object({
      index: z.coerce.number().optional().describe("Tab index to keep"),
      url: z.string().optional().describe("Exact URL to keep"),
      urlContains: z.string().optional().describe("Keep tabs whose URL contains this text"),
      titleContains: z.string().optional().describe("Keep tabs whose title contains this text"),
      status: z.string().optional().describe("Optional disposition label such as deliverable or handoff"),
    })).optional().describe("Tabs to keep open"),
  },
  async ({ keep = [] }) => {
    const rawTabs = await safari.listTabs();
    const tabs = _parseTabs(rawTabs);
    const closed = [];
    const released = [];
    for (const [trackedIndex, info] of [..._openedTabs.entries()]) {
      const tab = tabs.find(t => t.url && info.url && t.url === info.url) || tabs.find(t => Number(t.index) === Number(trackedIndex));
      if (!tab) {
        _untrackTab(trackedIndex);
        continue;
      }
      if (info.origin === "claimed" || _tabMatchesKeep(tab, keep)) {
        released.push({ ...tab, origin: info.origin || "created" });
        _untrackTab(trackedIndex);
        continue;
      }
      try {
        ...
        released.push({ ...tab, error: err.message });
      }
      _untrackTab(trackedIndex);
    }
    return { content: [{ type: "text", text: JSON.stringify({ closed, released }, null, 2) }] };
  }
);
...
server.tool(
  ...
  async ({ url }) => {
    ...
    const createdTabs = [..._openedTabs.entries()].filter(([, info]) => info.origin !== "claimed");
    if (createdTabs.length >= MAX_TABS) {
      let oldestIdx = null, oldestTime = Infinity;
      for (const [idx, info] of createdTabs) {
        if (info.openedAt < oldestTime) { oldestTime = info.openedAt; oldestIdx = idx; }
      }
      if (oldestIdx !== null) {
    ...
    // Sync session cleanup tracking even when the active transport omits tabIndex.
    await _trackOpenedTabFromCurrentState(url, result);
    if (result?.url || url) {
...
try {
  if (process.env.SAFARI_MCP_QUIET !== "1") {
    ...
    if (Date.now() - lastShown > ONE_DAY_MS) {
      ...
      console.error("");
      console.error(`[Safari MCP] 🦁 v${version} ready — native WebKit, zero Chrome.`);
      console.error(`[Safari MCP] ⭐ Like it? Star: https://github.com/abdousarr/codex-plugins`);
      console.error("");
