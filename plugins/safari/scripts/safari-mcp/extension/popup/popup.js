...
async function check() {
  ...
  try {
    ...
    if (resp) {
      if (resp.connected) { setStatus("connected", "Connected", "Port 9324"); return; }
      if (!resp.enabled) { setStatus("paused", "Paused", "Toggle to resume"); return; }
    }
  } catch {}
  // Fallback: read from storage
  const s = await getVal("mcpStatus", null);
  if (s === "connected") setStatus("connected", "Connected", "Port 9324");
  else if (s === "paused") setStatus("paused", "Paused", "Toggle to resume");
  else if (s === "checking") setStatus("checking", "Connecting...", "Trying port 9324...");
