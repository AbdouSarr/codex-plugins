// Codex Safari Bridge — Background Service Worker
...
const HTTP_URL = "http://127.0.0.1:9324";
...
self.addEventListener("unhandledrejection", (e) => {
  e.preventDefault();
  console.warn("Codex Safari Bridge: unhandled rejection:", e.reason);
});
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (data.profile) {
        ...
        if (!isCorrectProfile) {
          console.log(`Codex Safari Bridge: wrong profile — server wants "${data.profile}", disconnecting`);
          updateBadge("OFF");
...
async function pollForCommands() {
  while (isConnected && _enabled) {
    try {
      ...
      isConnected = false;
      updateBadge("");
      console.log("Codex Safari Bridge: poll failed, reconnecting...", err.message);
      scheduleReconnect();
      return;
    }
  }
...
async function executeAndReply(msg) {
  ...
  try {
    ...
    console.warn("Codex Safari Bridge: failed to send result to server:", err.message);
  }
}
    ...
    case "snapshot": {
      return await execInTab((rootSelector, snapshotGen) => {
        ...
        function walk(node, depth) {
          ...
          // a double-quote could otherwise break the pseudo-XML snapshot and inject a forged
  ...
  try {
    ...
    if (verifyRes && verifyRes.ok) {
      ...
      try {
        ...
        console.warn("Codex Safari Bridge: profile verification response invalid JSON — rejecting");
        return false;
      }
      if (result.match) {
    ...
    console.log("Codex Safari Bridge: profile verification inconclusive — rejecting connection");
    return false;
  } catch (err) {
    console.warn("Codex Safari Bridge: profile verification error:", err.message, "— rejecting connection");
...
async function _discoverProfileWindow() {
  ...
  try {
      ...
      try {
        ...
        if (stored.mcpProfileWindowId) {
          ...
          if (win) {
            ...
            console.log("Codex Safari Bridge: profile window restored from storage:", _profileWindowId);
            return;
          }
        }
      } catch (_) {}
    ...
    console.log("Codex Safari Bridge: profile window:", _profileWindowId);
  } catch (err) {
    console.warn("Codex Safari Bridge: _discoverProfileWindow error:", err.message);
  }
}
...
async function getTargetTab(tabUrl, sessionId) {
  ...
  if (tabUrl) {
    ...
    if (match) {
      ...
      if (!_profileWindowId || match.windowId !== _profileWindowId) {
        if (_profileWindowId && match.windowId !== _profileWindowId) {
          console.log("Codex Safari Bridge: profile window changed:", _profileWindowId, "→", match.windowId);
        }
        _profileWindowId = match.windowId;
        browser.storage.local.set({ mcpProfileWindowId: _profileWindowId }).catch(() => {});
        console.log("Codex Safari Bridge: profile windowId =", _profileWindowId);
      }
      return match;
    }
  }
  ...
  if (_profileWindowId) {
    ...
    console.warn("Codex Safari Bridge: profile window has no active tab, re-discovering...");
    await _discoverProfileWindow();
    if (_profileWindowId) {
...
// ========== STARTUP ==========
console.log("Codex Safari Bridge: service worker started");
updateBadge("");
