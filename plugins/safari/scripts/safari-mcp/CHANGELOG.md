...
- **Extension snapshot**: page-controlled attribute values (`aria-label`, `title`, `value`, `href`, …) are now HTML-escaped, so a crafted attribute can't inject a forged `ref=`/`role=` into the snapshot and steer the agent to the wrong element.
